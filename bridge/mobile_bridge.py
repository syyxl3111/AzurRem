...
用法：
    ...
    AzurRemBridge.exe "D:\\你的路径\\AzurPilot"
    # 源码方式跑：任意 Python 3.10+（只用标准库，**不需要** AzurPilot 的 venv）
    python mobile_bridge.py --root "%AZURPILOT_ROOT%" --port 25550
    # 默认监听 0.0.0.0:25550
...
def _common_root_candidates() -> list:
    ...
    刻意**不写死"某盘某目录"那种列表** —— 把某个具体盘符 + 具体目录名写死，
    等于把开发者的机器布局抄进仓库（而且 `_scan_drives_for_root()` 本来就会把每个盘的
    根目录和 `<盘>:\\Tools\\` 都扫一遍，再列一遍纯属重复）。

    所以这里只放两类真正与"当前这台机器"绑定的位置：环境变量显式指定的、以及
    当前用户目录下的常见落点。跨盘的猜测交给 `_scan_drives_for_root()`。
    """
    candidates = []
    env = (os.environ.get("AZURPILOT_ROOT") or "").strip()
    if env:
        # 显式指定的排最前：用户说的比任何猜测都准
...
# 出处：<AzurPilot 目录>\module\os\ship_exp_data.py
...
def _usage() -> str:
    return (
        ...
        "  AzurRemBridge.exe \"D:\\你的路径\\AzurPilot\"  也可以把 AzurPilot 文件夹拖上来\n"
