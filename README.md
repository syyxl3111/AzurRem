...
- **从托盘还原后，蓝点点了没反应** —— 左键点托盘图标把窗口叫回来后，再点蓝点收不回托盘。
  窗口虽然"可见"也"置顶"，但**不是前台窗口**，那一下左键会先被 Windows 当成
  "先激活我"吃掉，根本传不到按钮上。现在还原时会真正激活窗口。
...
**三步跑起来**（内网、公网填法完全一样，就这三步）：

1. 💻 **电脑** —— 到 [**Releases**](https://github.com/syyxl3111/AzurRem/releases/latest)
   下载 `AzurRemBridge.exe`，**双击**。窗口出来先别关。
2. 📱 **手机** —— 同一个页面下载 `AzurRem-1.0.4.apk`，装上打开。
3. 🔗 **连起来** —— App 里进 **设置**，把网关窗口那**两行**（地址 + 密码）抄进去。
   看到「已连接」就成了。

下面两节是这三步的细节和坑。装好之后每个 Tab 能干什么，见[功能](#功能)那一节。
