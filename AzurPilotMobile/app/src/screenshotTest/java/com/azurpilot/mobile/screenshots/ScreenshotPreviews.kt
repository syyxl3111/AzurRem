...
private fun res(
    ...
    limit = limit,
    total = total,
    lastUpdateRaw = if (never) "2020-01-01 00:00:00" else "2026-09-11 02:18:41",
    never = never,
)
...
).map { ScheduledTask(it.first, it.second, null) }
...
private val state = AppUiState(
    ...
    cl1Stats = Cl1Stats(
        ...
        sortieCost = 38_640,
        akashiEncounters = 7,
        averageStamina = 118.4,
        netStaminaGain = -212.6,
    ),
)
...
fun Logs() {
    AppTheme(darkTheme = false) {
        LogsScreen(state, onRefresh = {}, onClear = {}, insets = INSETS)
    }
}
...
fun Tasks() {
    AppTheme(darkTheme = false) {
        TasksScreen(state, onRefresh = {}, onTrigger = {}, insets = INSETS)
    }
}
...
fun Stats() {
    AppTheme(darkTheme = false) {
        StatsScreen(state, onRefresh = {}, insets = INSETS)
    }
}
